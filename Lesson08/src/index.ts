...
import express from "express" // Заменяем импорт через require
const app = express() // Далее мы создаём некое приложение app с помощью вызова функции express
const port = 3000 

app.get('/', (req, res) => { // метод “GET” Он будет реагировать на get-запрос на корневой адрес, куда будут приходить request и response
...
app.post('/samurais', (req, res) => { // если на эндпоинт “/samurais” придёт post-запрос, то мы скажем: “We have created samurai”.
    res.send('We have created samurai')
})

app.listen(port, () => {   // прилодение будет слушать port
